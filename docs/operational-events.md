...
| `planner-preview-rejected` | A valid decomposition preview was held for approval or explicitly rejected without inserting child nodes. Held previews record structured data in `pendingPlannerPreview`; rejected previews clear that metadata. | `previousStatus`, `status`, `session`, `runId`, `requestId`, `proposedKind`, `childIds`, `reason`, `report` |
| `planner-preview-regenerated` | A valid planner preview replaced older pending preview metadata on the same node. | `previousStatus`, `status`, `session`, `runId`, `previousRequestId`, `requestId`, `proposedKind`, `childIds`, `reason`, `report` |
...
Graph history event details pass through operational redaction before they are appended, and the `events` command redacts again while exporting. The redactor masks Slack webhook URLs, bearer tokens, URL userinfo such as `https://user:token@example/repo.git`, common `token`, `password`, `secret`, `api_key`, and `authorization` assignments, and unsafe planner raw-text keys such as `prompt`, `rawText`, `rawPrompt`, `rawPlannerText`, and `rawPlannerOutput`. Primary node fields such as `question`, `answer`, and `failureReason` still store the operator-provided value because they are part of task state; event payloads are the safer audit stream for broad inspection.
