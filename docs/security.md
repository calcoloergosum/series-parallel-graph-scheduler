...
| Planner output boundary | Code: planner responses are validated, materialized into normal graph nodes, and written only through the locked graph writer; prompt-backed planning uses an injected prompt adapter or the configured Codex command. Tests: goal planning, worker planner preflight, stale preview, invalid planner output, generated graph replay, and visualizer `/api/goal/plan` coverage. |
