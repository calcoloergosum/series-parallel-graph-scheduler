...
| Planner raw text and prompts | prompt-backed planner runtime results, planner-shaped graph metadata, graph history details | Raw planner prompts and raw planner output are memory-only runtime fields. Public planner decomposition results omit top-level `prompt` and `rawText`; worker approval/failure reports store only the parsed response, proposed mutation, validation summary, and operational reason. Graph history, exported events, and broad visualizer payloads replace planner raw-text fields such as `prompt`, `rawText`, `rawPrompt`, and `rawPlannerOutput` with `[REDACTED: planner text]`. Structured planner fields such as goal text, rationale, context refs, and decisions may remain visible and must be inserted with DOM text APIs or escaped HTML. |
...
- Planner sinks: prompt-backed planner command output, broad visualizer payloads, node history, event payloads, selected-node goal text, rationale, context refs, output contracts, and result summaries are covered by raw-text redaction and DOM-text rendering tests.
