...
| Planner | Valid decomposition preview while worker planner mode is `ask-approval` | Target remains a task leaf and moves to `blocked`; no preview child is inserted. | `planner-preview-rejected` plus `blocked` history entries; planner preflight report includes proposed children and decompose mutation. | Approve by manually decomposing from the report, or discard with `reset --node NODE`. |
