...
| 2 | Scheduler semantics | Readiness, claim, start, renew, done, block, answer, fail, reset, reset-subtree, reset-reachable, decompose, apply-preview, reject-preview, reconcile, and release-expired have a documented transition table and tests for allowed and important disallowed transitions. | A state-machine diagram rendered in docs. |
