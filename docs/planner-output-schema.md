...
- Child proposals have non-empty titles and supported proposed kinds.
- Planner-provided `id` values are deterministic, unique among siblings, absent
  from the target graph, and accepted by the scheduler safe-token policy.
