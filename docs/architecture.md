...
| `scripts/node-mutations.ts` | Scheduler state transitions: claim, start, renew, done, block, answer, fail, reset variants, decompose, planner preview apply/reject, reconcile, release expired leases. | Side-effectful because exported commands acquire locks, read graph files, mutate graph objects, and write files. Transition rules live here. |
