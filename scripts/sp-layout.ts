...
const defaultOptions = {
  ...
  framePadBottom: 18,
  margin: 18
} satisfies ResolvedLayoutOptions;

const edgeKinds = new Set<LayoutEdgeKind>(["series", "parallel", "frame"]);

export function buildPlanarLayout(graph: PlanGraphFile, options: LayoutOptions = {}): PlanarLayout {
  const opts: ResolvedLayoutOptions = { ...defaultOptions, ...options };
  ...
  function graphNode(nodeId: string): GraphNode {
  ...
  function offsetPoint(point: LayoutPoint, dx: number, dy: number): LayoutPoint {
    return { x: point.x + dx, y: point.y + dy };
  }

  function offsetComponent(component: LayoutComponent, dx: number, dy: number): LayoutComponent {
    return {
      ...component,
  ...
  function mergeComponents(parts: LayoutComponent[]): Pick<LayoutComponent, "boxes" | "frames" | "edges"> {
    return {
  ...
  function buildLeaf(nodeId: string): LayoutComponent {
  ...
  function buildSeries(children: LayoutComponent[]): LayoutComponent {
    if (children.length === 0) {
      throw new Error("Series layout requires at least one child");
    }
    ...
    let x = 0;
    let previousOutput: LayoutPoint | null = null;
  ...
  function buildParallel(children: LayoutComponent[]): LayoutComponent {
    if (children.length === 0) {
      throw new Error("Parallel layout requires at least one child");
    }
    if (children.length === 1) {
      return children[0];
    }
  ...
  function wrapFrame(nodeId: string, inner: LayoutComponent, depth: number): LayoutComponent {
    const node = graphNode(nodeId);
  ...
  function visit(nodeId: string, stack: string[] = [], depth = 0): LayoutComponent {
  ...
  return {
    ...
    boxes: root.boxes,
    frames: root.frames,
    edges: root.edges.filter((edge): edge is LayoutEdge & { kind: LayoutEdgeKind } => edgeKinds.has(edge.kind as LayoutEdgeKind))
  };
}

export function renderPlanarSvg(graph: PlanGraphFile, options: LayoutOptions = {}): string {
...
function renderFrames(frames: LayoutFrame[]): string[] {
  return [...frames]
...
function renderTerminal(point: LayoutPoint, label: string): string {
